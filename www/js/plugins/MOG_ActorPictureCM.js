...
Actor_CMPicture.prototype.refreshBitmapCM2 = function () {
	//アクターのIDを取得し、アクターIDが２０以下なら脱衣などの差分を作る。
	//アクターIDが９番未満なら一桁、それ以降なら二桁の数字を取る
	if (isNaN(this.fileName().slice(-2))) {
		var actorNum = Number(this.fileName().slice(-1));
	} else {
		var actorNum = Number(this.fileName().slice(-2));
	}
	if (actorNum < 21) {
		var fileName = this.fileName();
		//HPが50%以上の場合「1」50%未満25%以上の場合[2]それ未満の場合[3]を入れる
		if ($gameActors.actor(actorNum).hp > $gameActors.actor(actorNum).mhp * 0.5) {
			fileName += "_1";
		} else if ($gameActors.actor(actorNum).hp > $gameActors.actor(actorNum).mhp * 0.25) {
			fileName += "_2";
		} else {
			fileName += "_3";
		}
		//ステート12の場合は「2」,13の場合は「3」、どちらでもない場合は「1」を入れる
		if ($gameActors.actor(actorNum).isStateAffected(13)) {
			fileName += "3";
		} else if ($gameActors.actor(actorNum).isStateAffected(12)) {
			fileName += "2";
		} else {
			fileName += "1";
		}
		//ステート14の場合は「2」,15の場合は「3」、どちらでもない場合は「1」を入れる
		if ($gameActors.actor(actorNum).isStateAffected(15)) {
			fileName += "3";
		} else if ($gameActors.actor(actorNum).isStateAffected(14)) {
			fileName += "2";
		} else {
			fileName += "1";
		}
		//ステート22の場合は「2」そうでない場合は「1」を入れる
		if ($gameActors.actor(actorNum).isStateAffected(22)) {
			fileName += "2";
		} else {
			fileName += "1";
		}
