...
(function () {
    ...
    Window_EquipStatus.prototype.drawParamGauge = function (x, y, width, curValue, newValue, max, down) {
        var count = Math.min(this._gaugeCount, 22);
        var curWidth = width * curValue / max;
        var value = curValue + (newValue - curValue) * Math.sqrt(22 - count) / Math.sqrt(22);
    ...
    Window_EquipStatus.prototype.drawParam = function (x, y, paramId) {
        ...
        var down = MPPlugin.downStrongParams.contains(paramId);
        this.drawParamGauge(x + 140, y, 130, curValue, newValue, max, down);
    ...
    Window_EquipStatus.prototype.drawNewParam = function (x, y, paramId) {
        if (paramId < 8) {
            ...
            if (MPPlugin.downStrongParams.contains(paramId)) {
                diffvalue *= -1;
            }
