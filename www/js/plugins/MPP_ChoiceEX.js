...
(function () {
    ...
    Game_Interpreter.prototype.evalChoice = function (formula) {
        try {
            ...
            var v = $gameVariables._data;//修正
            return !!eval(formula);//修正
            //var formula2 = formula.replace(/v\[(\d+)\]/g,//修正
            //    (match, p1) => $gameVariables.value(parseInt(p1)) );//修正
            //return !!eval(formula2);//修正
        } catch (e) {
            alert("条件エラー \n\n " + formula);
            return true;
        }
